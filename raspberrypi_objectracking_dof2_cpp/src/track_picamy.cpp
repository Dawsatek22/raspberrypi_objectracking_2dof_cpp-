...
camera(module 2 currently)and  to track the yellow color in 
the y direction with the servos
*/
...
track.set_pwm_freq(60.0); // current frequency
servoval = mapservo(pulsval,0,180,SERVOMIN,SERVOMAX);
track.set_pwm(piny,180,servoval); //  servos is calibrated to prefeered position
...
// below checks if the frame video frame is availabe
 ...
 int error = y_medium/6; // supossed to be the offset
//position = error;
...
std::cout << "position of servos is reached 180" << '\n';
 ...
 track.set_pwm(piny,position,servoval); // moves servos acording to the position value
std::cout << "angle of y direction ="<< position <<'\n';
